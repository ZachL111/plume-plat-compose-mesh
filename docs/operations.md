...
`plume-plat-compose-mesh` is centered on platform engineering work in Java. The technical target is: Package a Java local lab for compose analysis with node-edge fixtures, cycle and reachability reports, and documented operating limits.
...
The score is deterministic. The current threshold is 155. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 5.
